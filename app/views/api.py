...
@ api_view(['GET', 'PUT', 'DELETE'])
def uploadLabelingPoseById(request):
    if request.method == 'PUT' and request is not None:
        request_body = request.data
        request_dict = request_body.dict()
        pose_data = request_dict.get("pose")
        bag_id = request_dict.get("bagId")
        print("data is  %s " % pose_data)
        return JsonResponse(db.upload_labeling_pose_by_id(pose_data, bag_id), safe=False)
...
@ api_view(['GET', 'PUT', 'DELETE'])
def getLabelingPoseById(request):
    if request.method == 'PUT':
        if request.data is not None:
            print("into get labeling data")
            print(request.data.dict())
            return JsonResponse({'result': db.get_labeling_pose_data(request.data)})
